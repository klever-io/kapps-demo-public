...
export const SelectorContainer = styled.div<{ open: boolean }>`
  ...
  &:hover {
    &::-webkit-scrollbar-thumb {
      background: ${props => transparentize(0.75, props.theme.background)};
    }
  }
`;
...
export const Item = styled.div<{ selected: boolean }>`
  ...
  background-color: ${props =>
    props.selected && transparentize(0.4, props.theme.card.background)};
    ...
    ${props =>
      props.selected
        ? css`
          ...
          `
        : css`
            color: ${props.theme.filter.item.text};
            font-weight: 400;
          `};
  }
`;
